...
export const tasks = sqliteTable("tasks", {
  ...
  priority: text("priority", { enum: ["LOW", "MEDIUM", "HIGH"] })
    .default("MEDIUM")
    .notNull(),
  category: text("category", { enum: ["WORK", "HEALTH", "FUN", "CHORE"] })
    .default("CHORE")
    .notNull(),
...
// Game High Scores Table (Leaderboards)
export const gameHighScores = sqliteTable(
  "game_high_scores",
  {
    id: text("id").primaryKey(),
    userId: text("user_id")
    ...
    gameId: text("game_id").notNull(), // e.g., '2048', 'sixfold-isle'
  ...
  (table) => {
    return {
      userGameUniqueIdx: uniqueIndex("user_game_idx").on(
        table.userId,
        table.gameId,
      ),
      gameScoreIdx: index("game_score_idx").on(table.gameId, table.highScore),
    };
  },
);
