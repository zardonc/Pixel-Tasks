...
export const Layout: React.FC<LayoutProps> = ({ children, onAddTask }) => {
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark relative overflow-x-hidden transition-colors duration-200">
      ...
      >
        {/* Drawer Header */}
        <div className="mb-6 border-b-2 border-black dark:border-white pb-4 border-dashed">
          <h2 className="text-3xl font-bold tracking-wider uppercase">Menu</h2>
        ...
        {/* Navigation */}
        <nav className="flex-1 space-y-1 overflow-y-auto pr-2 pb-2">
          <DrawerItem
            to="/"
        ...
        {/* Bottom Actions */}
        <div className="mt-auto pt-4 border-t-2 border-black dark:border-white border-dashed space-y-3">
          {/* Dark Mode Toggle */}
          <button
            onClick={toggleDarkMode}
            className="w-full flex items-center gap-4 text-lg font-bold py-2 text-gray-600 dark:text-gray-300 hover:pl-6 transition-all duration-200"
          >
...
const DrawerItem = ({
  ...
  <NavLink
    ...
    onClick={onClick}
    className={({ isActive }) => `
          flex items-center gap-4 text-lg font-bold py-2 px-2 transition-all duration-200 group rounded
          ${
            isActive
              ? "bg-gray-100 dark:bg-gray-800 text-black dark:text-white pl-4 border-l-4 border-black dark:border-white"
              : "text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:pl-4"
          }
      `}
  >
